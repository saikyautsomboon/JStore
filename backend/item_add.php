...
$subcategoryid = trim($_POST['subcategoryid']);
    ...
    for ($i=0; $i < $secphoto ; $i++) { 
        ...
        $secsql="INSERT INTO item_detail(photo,item_id,create_at,update_at) VALUES (:value1,:value2,:value3,:value4)";
        $secstmt=$conn->prepare($secsql);

        $secstmt->bindParam(':value1', $targetpath);
        $secstmt->bindParam(':value2', $itemsid);
        $secstmt->bindParam(':value3', '23');
        $secstmt->bindParam(':value4', '34');
        $secstmt->execute();
